...
def my_EnKF(state, obs, N, scaling):
    ...
    Perturbs = scaling * R12 @ rnd.randn(100, ensemble_size)
...
while (time_step < max_time_step):
    ...
    if (time_step % anim_interval == 0):
        scaling = 1.5
        with torch.no_grad():
            target_true = torch.Tensor(np.stack([u_list[time_step // anim_interval - 2], v_list[time_step // anim_interval - 2], eta_list[time_step // anim_interval - 2]], axis = 0)).unsqueeze(0).view(1, -1).to(device)
            target = torch.Tensor(np.stack([u_n, v_n, eta_n], axis = 1)).view(u_n.shape[0], -1).to(device)
            x_hat_current, mean_current, log_var_current = model(target)
            x_hat, mean, log_var = model(target_true)
            x_hat_sample, mean_sample, log_var_sample = model.forward_sample(target_true[:, ::25])
            latent = model.decode(torch.Tensor(my_EnKF(mean_current.detach().cpu().numpy().T, mean.detach().cpu().numpy().reshape(-1) + R12 @ rnd.randn(R12.shape[0]), ensemble_size, scaling)).to(device).T.reshape(ensemble_size, -1)).reshape(ensemble_size, 3, 150, 150).detach().cpu().numpy()
            eta_n = latent[:, 2, :, :]
            u_n = latent[:, 0, :, :]
            v_n = latent[:, 1, :, :]
    # Samples for Hovmuller diagram and spectrum every sample_interval time step.
...
eta_anim = viz_tools.eta_animation_overlay(X, Y, eta_list, eta_list_enkf, eta_list, eta_list, anim_interval*dt, "eta")
