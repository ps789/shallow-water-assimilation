...
def my_EnKF(state, obs, N, scaling):
    ...
    Perturbs = scaling * R12 @ rnd.randn(xDim, ensemble_size)
...
while (time_step < 0):#max_time_step):
    ...
    if (time_step % anim_interval == 0):
        scaling = 1.5
        eta_n = my_EnKF(eta_n.reshape((eta_n.shape[0], -1)).T, eta_list[time_step // anim_interval - 2].reshape(-1) + R12 @ rnd.randn(xDim), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
        u_n = my_EnKF(u_n.reshape((u_n.shape[0], -1)).T, u_list[time_step // anim_interval - 2].reshape(-1) + R12 @ rnd.randn(xDim), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
        v_n = my_EnKF(v_n.reshape((v_n.shape[0], -1)).T, v_list[time_step // anim_interval - 2].reshape(-1) + R12 @ rnd.randn(xDim), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
    # Samples for Hovmuller diagram and spectrum every sample_interval time step.
    if (time_step % sample_interval == 0):
...
while (time_step < max_time_step):
    ...
    if (time_step % anim_interval == 0):
        obs = torch.Tensor(np.arctan(eta_list[time_step // anim_interval - 2]) + obs_sigma *  rnd.randn(N_x, N_y)).to(device)
