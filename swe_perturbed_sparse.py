...
N_x = 70                            # Number of grid points in x-direction
N_y = 70                            # Number of grid points in y-direction
...
while (time_step < max_time_step):
    ...
    if (time_step % anim_interval == 0):
        ...
        rmse_temp = np.sqrt(np.mean((eta_n - eta_list[time_step // anim_interval - 2])**2))
        print("RMSE")
        print(rmse_temp)
...
obs_sigma =  0.005
R12 = obs_sigma*np.eye((N_x * N_y) // 10)
R = R12 @ R12.T
def Obs(E):
    return E
...
def my_EnKF(state, obs, N, scaling):
    ...
    Perturbs = scaling * R12 @ rnd.randn(xDim // 10, ensemble_size)
...
while (time_step < max_time_step):
    ...
    if (time_step % anim_interval == 0):
        scaling = 1.5
        eta_n = my_EnKF(eta_n.reshape((eta_n.shape[0], -1)).T, eta_list[time_step // anim_interval - 2].reshape(-1)[::10] + R12 @ rnd.randn(R12.shape[0]), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
        u_n = my_EnKF(u_n.reshape((u_n.shape[0], -1)).T, u_list[time_step // anim_interval - 2].reshape(-1)[::10] + R12 @ rnd.randn(R12.shape[0]), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
        v_n = my_EnKF(v_n.reshape((v_n.shape[0], -1)).T, v_list[time_step // anim_interval - 2].reshape(-1)[::10] + R12 @ rnd.randn(R12.shape[0]), ensemble_size, scaling).T.reshape(ensemble_size, N_x, N_y)
    # Samples for Hovmuller diagram and spectrum every sample_interval time step.
    if (time_step % sample_interval == 0):
    ...
    if (time_step % anim_interval == 0):
        ...
        rmse_temp = np.sqrt(np.mean((np.mean(eta_n, axis = 0) - eta_list[time_step // anim_interval - 2])**2))
        print("RMSE")
        print(rmse_temp)
...
while (time_step < max_time_step):
    ...
    if (time_step % anim_interval == 0):
        ...
        def score_likelihood(xt, t):
            ...
            logprob = -torch.sum(torch.square(xt_tensor[:, :, ::10] - obs)/obs_sigma**2/2)
            logprob.backward()
            score_x = xt_tensor.grad
            tau = g_tau(t)
            return tau*score_x
        eta_n = reverse_SDE(x0=eta_n, score_likelihood=score_likelihood, time_steps=euler_steps).detach().cpu().numpy()

        obs = torch.Tensor(np.arctan(u_list[time_step // anim_interval - 2])+ obs_sigma * rnd.randn(N_x, N_y)).to(device)
        def score_likelihood(xt, t):
            # obs: (d)
            # xt: (ensemble, d)
            # the line below is analytical ∇z log P(Yt+1|z)
            ...
            # print(score_x)
            # xt_tensor = torch.autograd.Variable(torch.Tensor(xt).to(device), requires_grad = True)
            # logprob = -torch.sum(torch.square(xt - obs)/obs_sigma**2/2)
            # logprob.backward()
            # score_x = xt_tensor.grad
            # print(score_x)
            tau = g_tau(t)
            return tau*score_x
        u_n = reverse_SDE(x0=u_n, score_likelihood=score_likelihood, time_steps=euler_steps).detach().cpu().numpy()

        obs = torch.Tensor(np.arctan(v_list[time_step // anim_interval - 2])+ obs_sigma *  rnd.randn(N_x, N_y)).to(device)
        def score_likelihood(xt, t):
            # obs: (d)
            # xt: (ensemble, d)
            # the line below is analytical ∇z log P(Yt+1|z)
            ...
            tau = g_tau(t)
            return tau*score_x
        v_n = reverse_SDE(x0=v_n, score_likelihood=score_likelihood, time_steps=euler_steps).detach().cpu().numpy()
    # Samples for Hovmuller diagram and spectrum every sample_interval time step.
    if (time_step % sample_interval == 0):
        hm_sample.append(np.mean(eta_n, axis = 0)[:, int(N_y/2)])              # Sample middle of domain for Hovmuller
        ts_sample.append(np.mean(eta_n, axis = 0)[int(N_x/2), int(N_y/2)])     # Sample center point for spectrum
