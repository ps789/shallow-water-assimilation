...
while (time_step < max_time_step):
    ...
    # Store eta and (u, v) every anin_interval time step for animations.
    if (time_step % anim_interval == 0):
        target = np.stack([u_n, v_n, eta_n], axis = 0)[np.newaxis, ...].reshape(1, -1)
        t_rep = torch.Tensor([time_step*dt/21/5000]*target.shape[0]).to(device)
        decoded = np.dot(np.dot(target - PCA_means, PCA_V), PCA_V.T)+PCA_means
        u_n_decoded, v_n_decoded, eta_n_decoded = decoded[0, :150*150].reshape(150, 150), decoded[0, 150*150:2*150*150].reshape(150, 150), decoded[0, 150*150*2:150*150*3].reshape(150, 150)
        print("Time: \t{:.2f} hours".format(time_step*dt/3600))
        print("Step: \t{} / {}".format(time_step, max_time_step))
        print("Mass: \t{}\n".format(np.sum(eta_n)))
