...
for pbar in tqdm(range(200)):
    ...
    for _ in range(50):
        ...
        # ==================================================================================
        while (time_step < max_time_step or record_flag):
            ...
            if ((random.random() < 0.001 or time_step == 1) and not record_flag):
                record_flag = True
                x_data_sequence = []
                y_data_sequence = []
                t_data_sequence = []
            if record_flag:
                x_data_sequence.append(np.stack([u_n, v_n, eta_n], axis = 0))
                t_data_sequence.append(time_step*dt)
                y_data_sequence.append(output)
                if len(x_data_sequence) >= 20:
                    record_flag = False
                    x_data.append(x_data_sequence)
                    y_data.append(y_data_sequence)
                    t_data.append(t_data_sequence)

    x_final = np.stack(x_data, axis = 0)
    t_final = np.stack(t_data, axis = 0)
    y_final = np.stack(y_data, axis = 0)
    np.save(f"./sequential_data/x_data_sequential_{pbar}.npy", x_final, allow_pickle=True)
    np.save(f"./sequential_data/y_data_sequential_{pbar}.npy", y_final, allow_pickle=True)
    np.save(f"./sequential_data/t_data_sequential_{pbar}.npy", t_final, allow_pickle=True)
